...
3.  **Run the Docker container.**
    ...
    - _(Note: You'll need a [Hugging Face User Access Token](https://huggingface.co/settings/tokens) with "read" permissions. This is required for the libraries to download the models without being rate-limited.)_
...
2.  Navigate to the `frontend` folder from the root of your project:
    ...
    _(Or, from a new terminal: `cd C:\...YOUR_REPOSITORY_NAME\frontend`)_
...
2.  **Add Secret:** In the HF Space "Settings" tab, add your `HF_TOKEN` to the "Repository secrets".
    - _Troubleshooting:_ If you get a `401 Unauthorized` error in your logs, your token is invalid. Delete it, generate a new one, and add it. If that still fails, deleting the secret entirely will allow anonymous downloading, as all models are public.
