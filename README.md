...
cd backend
Build the Docker image. This will take a few minutes as it installs Python 3.12 and all the required libraries.
...
(Note: You'll need a Hugging Face User Access Token with "read" permissions. This is required for the libraries to download the models without being rate-limited.)

Bash

docker run --rm -p 8000:8000 -e HF_TOKEN=YOUR_HF_TOKEN_HERE --name rag-api rag-api-backend
Wait for the models to load. The backend is ready when you see this log: INFO: Uvicorn running on http://0.0.0.0:8000 (Press CTRL+C to quit)

Step 3: Run the Frontend (Terminal 2)
Open a new, separate terminal.

Navigate to the frontend folder from the root of your project:
...
(Verification) Make sure your frontend/app.py is pointing to your local server: API_URL = "http://127.0.0.1:8000/query"
...
Deploy Backend: Upload all 4 files from the backend folder (Dockerfile, requirements.txt, main.py, data.txt) to a new Docker SDK Hugging Face Space.

Add Secret: In the HF Space "Settings" tab, add your HF_TOKEN to the "Repository secrets".
...
Connect Frontend: Once the Space is "Running", copy its public URL (e.g., https://your-space-name.hf.space) and paste it into the API_URL variable in your local frontend/app.py file.

Relaunch your local streamlit run app.py. It will now be powered by your cloud-hosted backend.
